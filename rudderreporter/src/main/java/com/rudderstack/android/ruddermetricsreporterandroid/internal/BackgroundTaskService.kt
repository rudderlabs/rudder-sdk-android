...
enum class TaskType {
    ...
    /**
     * A task that sends a Database request.
     */
    DB_REQUEST,

    /**
     * A task that performs I/O, such as reading a file on disk. This should NOT include operations
     * related to error/session storage - use [ERROR_REQUEST] or [DB_REQUEST] instead.
     */
    IO,
...
class BackgroundTaskService @JvmOverloads constructor(
     ...
     * with the task type [TaskType.ERROR_REQUEST], [TaskType.DB_REQUEST] and [TaskType.IO].
    ...
    fun <T> submitTask(taskType: TaskType, callable: Callable<T>): Future<T> {
        ...
        when (taskType) {
            ...
            TaskType.DB_REQUEST -> databaseExecutor.execute(task)
    ...
    fun shutdown() {
        ...
        databaseExecutor.awaitTerminationSafe()
