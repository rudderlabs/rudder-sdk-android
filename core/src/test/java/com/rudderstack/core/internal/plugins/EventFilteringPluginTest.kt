...
class EventFilteringPluginTest{
    ...
    fun testIntercept_AllowAllEventsWhenNoServerConfig() {
        ...
        val demoMessage = TrackMessage.create("testEvent", anonymousId = "anon-1", timestamp =
        Date().toString())
...
//        whenever(chain.plugins).thenReturn(dummyPlugins)
    ...
    fun `allow all destination plugins when event name not in black list`() {
            ...
            source = RudderServerConfig.RudderServerConfigSource(
                destinations = listOf(
                    RudderServerConfig.RudderServerDestination(
                        ...
                        destinationConfig = mapOf("eventFilteringOption" to "blacklistedEvents",
                            "blacklistedEvents" to setOf(mapOf("eventName" to "test1"), mapOf("eventName" to
                                    "test2", ), mapOf("eventName" to "test3"))
                        )
                    )
                )
            )
        ...
        whenever(chain.plugins).thenReturn(listOf(
        ...
        MatcherAssert.assertThat(chainCopy, allOf(
            org.hamcrest.Matchers.notNullValue(),
            org.hamcrest.Matchers.instanceOf(CentralPluginChain::class.java),
            org.hamcrest.Matchers.hasProperty("plugins",
            allOf( Matchers.iterableWithSize<Plugin>(2), Matchers.hasItem(
                allOf( Matchers.instanceOf < BaseDestinationPlugin < * > >
    ...
    fun `disallow destination plugins when event name in black list`() {
        ...
        val demoMessage = TrackMessage.create("test1", anonymousId = "anon-1", timestamp =
        Date().toString())
        whenever(chain.message()).thenReturn(demoMessage)
        //destination plugin list
        whenever(chain.plugins).thenReturn(listOf(
        ...
        MatcherAssert.assertThat(chainCopy, allOf(
            ...
            org.hamcrest.Matchers.instanceOf(CentralPluginChain::class.java),
            org.hamcrest.Matchers.hasProperty("plugins",
            allOf( Matchers.iterableWithSize<Plugin>(1), not(Matchers.hasItem(
                allOf( Matchers.instanceOf < BaseDestinationPlugin < * > >
                    (BaseDestinationPlugin::class.java), hasProperty("name", equalTo("Firebase"))
                )
            )),Matchers.hasItem(
            ...
            source = RudderServerConfig.RudderServerConfigSource(
                destinations = listOf(
                    RudderServerConfig.RudderServerDestination(
                        ...
                        isDestinationEnabled = true,
                        updatedAt = "2022-01-01",
                        destinationConfig = mapOf("eventFilteringOption" to "whitelistedEvents",
                            "whitelistedEvents" to setOf(mapOf("eventName" to "test1"), mapOf
                                ("eventName" to
                                    "test2", ), mapOf("eventName" to "test3"))
                        )
                    ), RudderServerConfig.RudderServerDestination(
                        ...
                        updatedAt = "2022-01-01",
                        destinationConfig = mapOf("eventFilteringOption" to "blacklistedEvents",
                            "blacklistedEvents" to setOf(mapOf("eventName" to "test2"), mapOf
                                ("eventName" to
                                    "test2", ), mapOf("eventName" to "test3"))
                        )
                    )
            ...
            source = RudderServerConfig.RudderServerConfigSource(
                destinations = listOf(
                    RudderServerConfig.RudderServerDestination(
                        ...
                        updatedAt = "2022-01-01",
                        destinationConfig = mapOf("eventFilteringOption" to "whitelistedEvents",
                            "whitelistedEvents" to setOf(mapOf("eventName" to "test1"), mapOf("eventName" to
                                    "test2", ), mapOf("eventName" to "test3"))
                        )
                    )
                )
