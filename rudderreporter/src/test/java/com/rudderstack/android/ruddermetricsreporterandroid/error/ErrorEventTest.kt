...
abstract class ErrorEventTest {
    ...
    fun serialize() {
        ...
        val actual = jsonAdapter.readJson(
            errorEvent.serialize(jsonAdapter)!!.also { println(it) },
        ...
        MatcherAssert.assertThat(
            actual?.entries?.filter {
                it.key != "exceptions"
            }?.also {
                println("parser: ${jsonAdapter::class.simpleName}")
                println(it)
            },
            contains(
                *(
